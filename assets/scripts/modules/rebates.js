...
$("#downloadSummaryButton").onclick = () => {
  ...
  // Espera el cálculo de estilos, gradientes y gráficas antes de abrir la impresión.
  report.setTimeout(() => {
    report.requestAnimationFrame(() => {
      report.focus();
      report.print();
    });
  }, 700);
...
$("#deletePartnerButton").onclick = () => {
  const partner = currentPartner();
  if (partner && confirm(`¿Archivar el aliado ${partner.name}? Dejará de aparecer en la cartera activa, pero se conservará su historial comercial.`)) {
