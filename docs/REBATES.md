...
4. Implemente el proyecto como aplicación web: **Implementar → Nueva implementación → Aplicación web**. Ejecútelo como la cuenta de DICOL y dé acceso a los usuarios autorizados de la organización. Copie la URL que termina en **`/exec`** en `APPS_SCRIPT_URL` de `assets/scripts/modules/rebates.js`; no use la URL de edición ni `/dev`.
5. Publique el sitio web después de cambiar esa URL. Si ve un error 404, la implementación fue eliminada o la URL copiada no corresponde a una aplicación web: cree una nueva implementación y repita este paso. Una implementación nueva puede usar el mismo código, pero siempre requiere comprobar y actualizar la URL configurada.
6. Después de cambios en Apps Script, cree una nueva implementación para publicar el código actualizado y pruebe el acceso con un usuario administrador y un especialista.
